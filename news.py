...
# ฟังก์ชันแก้ไขเครื่องหมาย ":" ที่ซ้ำหรือผิดรูปแบบในข้อความ
# ใช้ regex (COLON_RX) ค้นหาลักษณะของ colon ที่ไม่ปกติ เช่น "::", " : ", "：", ":-" ฯลฯ
# แล้วแทนให้เหลือเป็น ":" แบบเดียว เพื่อให้ข้อความเรียบร้อยและสม่ำเสมอ
# ใช้ในขั้นตอนทำความสะอาดข้อความก่อนส่งไป LINE หรือให้ Gemini วิเคราะห์
...
# ฟังก์ชันดึง URL รูปภาพจากบทความข่าว
# ปัจจุบันยังไม่ทำงานจริง (เป็น placeholder) และคืนค่าเป็นสตริงว่าง ""
# โครงสร้างนี้เตรียมไว้สำหรับอนาคต เช่น:
#   - ดึงรูปจาก meta tag (og:image / twitter:image)
#   - หรือดึงรูปหัวข่าวจาก HTML ของเพจ
# แต่ตอนนี้ยังไม่ใช้งาน จึง return "" เพื่อบอกว่าไม่มีรูป
...
- บริษัทหลักที่เกี่ยวข้อง: 
  - PTTEP (Upstream)
  - PTTLNG (นำเข้า/จัดเก็บ LNG)
  - PTTGL/ระบบท่อก๊าซ
  - PTTNGD (จัดจำหน่ายก๊าซ)
  - TTM (Trans Thai-Malaysia) โครงการท่อก๊าซและโรงแยกก๊าซร่วมไทย–มาเลเซีย เป็นบริษัทร่วมทุน 50:50 ระหว่าง PTT กับ PETRONAS ดูแล
    ท่อก๊าซ Thai–Malaysia Gas Pipeline และ Thai–Malaysia Gas Separation Plant ทั้งฝั่งไทยและมาเลเซีย
