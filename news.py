# -*- coding: utf-8 -*-
"""
Enhanced News Aggregator with Improved Deduplication
ระบบรวบรวมข่าวพลังงานที่ปรับปรุงการกันข่าวซ้ำ
"""
...
import feedparser
import pytz
from dateutil import parser as dateutil_parser# -*- coding: utf-8 -*-
"""
Enhanced News Aggregator with Improved Deduplication
ระบบรวบรวมข่าวพลังงานที่ปรับปรุงการกันข่าวซ้ำ
"""
...
class EnhancedDeduplication:
    ...
    def is_duplicate_content(self, item: dict) -> Tuple[bool, Optional[str]]:
        ...
        # บันทึก title ไว้เปรียบเทียบ
        norm_title = self.normalize_text(title)
        self.title_cache.append((norm_title, title))
        
        return False, None
    
    def score_item(self, item: dict) -> int:
        """ให้คะแนนความน่าเชื่อถือของข่าว"""
        score = 0
        
        url = item.get('canon_url') or item.get('url', '')
        if 'news.google.com' not in url and url:
            score += 15
        ...
        trusted_sources = [
            'reuters.com', 'bloomberg.com', 'bangkokpost.com',
            'thansettakij.com', 'prachachat.net'
        ]
        domain = item.get('domain', '').lower()
        if any(source in domain for source in trusted_sources):
            score += 20
...
# =============================================================================
# WTI FUTURES MODULE (เพิ่มใหม่)
# =============================================================================
class WTIFuturesFetcher:
    """ดึงข้อมูลราคา WTI Futures"""
    
    def fetch_mock_futures_data(self, current_price: float = 58.32) -> List[dict]:
        """สร้างข้อมูล futures ล่วงหน้า 12 เดือน"""
        futures_data = []
        now = datetime.now(TZ)
        ...
        for i in range(12):
            ...
            premium = i * 0.3
            future_price = current_price + premium
            
            import random
            fluctuation = random.uniform(-0.5, 0.5)
            future_price += fluctuation
    ...
    def get_current_and_futures(self) -> dict:
        ...
        current_price = 58.32  # ในการใช้งานจริง ควรดึงจาก API
        ...
        futures_data = self.fetch_mock_futures_data(current_price)
...
class WTIFlexMessageBuilder:
    ...
    def create_wti_futures_message(data: dict) -> dict:
        """สร้าง Flex Message แสดงราคา WTI Futures"""
        ...
        # Futures Rows
        futures_rows = []
        for i, future in enumerate(futures[:6]):
...
def main():
    print("="*60)
    print("ระบบติดตามข่าวพลังงาน + WTI Futures")
    print("="*60)
    ...
    # Mark as sent
    if success_news and not DRY_RUN:
        for item in news_items:
            append_sent_link(item.get('canon_url') or item.get('url'))
        print("\n[SUCCESS] อัปเดตฐานข้อมูลข่าวที่ส่งแล้ว")
    
    print("\n" + "="*60)
    print(f"ดำเนินการเสร็จสิ้น - ส่งข่าว: {success_news}, ส่ง WTI: {success_wti}")
    print("="*60)
