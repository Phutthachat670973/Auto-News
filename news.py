...
def create_flex_message(news_items):
    ...
    return [{
        "type": "flex",
        "altText": f"วิเคราะห์ข่าวเศรษฐกิจ/พลังงาน {today_thai.strftime('%d/%m/%Y')}",
...
def send_flex_to_line(flex_messages):
    ...
    for i, msg in enumerate(flex_messages):
        print(f"📦 ส่งชุดที่ {i+1}/{len(flex_messages)} ข่าว {len(msg['contents']['contents'])} เรื่อง")
