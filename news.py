...
2. ให้คะแนนความสำคัญของข่าวนี้ต่อกลุ่ม ปตท. (1-5 คะแนน)
   - แจกแจงว่าได้แต่ละคะแนนเพราะอะไร โดยระบุ "ปัจจัย" + "เหตุผล" ชัดเจน (ห้ามใช้คำกว้าง ๆ)
   - ตัวอย่าง:
...
3. วิเคราะห์ผลกระทบต่อบริษัทในกลุ่ม PTT (อิง Context)
   ...
   - ระบุ **ทิศทาง** (บวก/ลบ/ไม่ชัดเจน) และ **ระยะเวลา** (สั้น/กลาง/ยาว) ข้างชื่อบริษัทในวงเล็บ
   - อธิบายกลไกแบบเฉพาะเจาะจง เช่น:
     - PTTEP: รายได้ upstream/ความเสี่ยงแหล่งผลิต/ท่อ/ความไวต่อราคา
     - PTTLNG: โควตานำเข้า/ต้นทุนรีก๊าซ/ข้อจำกัดโครงสร้างพื้นฐานในไทย
     - PTTGL: margin/ความเสี่ยงพอร์ต LNG (SPA/offtake/สเปรด JKM/TTF/HH)
     - PTTNGD: ดีมานด์/ราคาปลายทาง/การขยายท่อและลูกค้า
   - **จำกัดจำนวนบริษัทไม่เกิน 2 ราย** เว้นแต่มีเหตุผลเฉพาะเจาะจงสำหรับทุกราย (ห้ามใส่ทุกบริษัทเพราะเป็นเมกะเทรนด์กว้าง ๆ)

4. แสดงผลลัพธ์ในรูปแบบ **ด้านล่างนี้เท่านั้น**:
- สรุปข่าว: <ข้อความ>
- คะแนน: <คะแนนรวมตัวเลข> (<คะแนนย่อย> จาก..., ...)
- ผลกระทบต่อ ปตท.: กระทบต่อ <ชื่อบริษัท 1 (ทิศทาง, ระยะเวลา) , ชื่อบริษัท 2 (ทิศทาง, ระยะเวลา)> เพราะ <เหตุผลสั้น>
- เหตุผลคะแนนรวม:
  - <คะแนน> คะแนน: <เหตุผล>
  - <คะแนน> คะแนน: <เหตุผล>
...
- ถ้าเป็นรายงาน/เมกะเทรนด์ (เช่น IEA/IRENA) ให้ถือเป็นผล **เชิงกลยุทธ์ทางอ้อม** และเลือกเพียงบริษัทที่มีเหตุผลเฉพาะ (ปกติไม่เกิน 1-2 ราย)
